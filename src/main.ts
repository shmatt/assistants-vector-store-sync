...
export async function run(): Promise<void> {
  try {
    ...
    const keyFiles = files.data.filter(
      (file: FileObject) =>
    ...
    do {
      vectorStore = vectorStoresReponse.data.find(
        (store: VectorStore) => (<any>store.metadata)['key'] == key
      )
    ...
    if (vectorStore == null) {
      ...
      core.info(
        `Found vector store: ${vectorStore.name} with id: ${vectorStore.id}`
      )
    }
    ...
    let vectorFilesResponse = await openai.beta.vectorStores.files.list(
      vectorStore.id
    )
    ...
    for await (const file of globber.globGenerator()) {
      try {
        ...
        const existing = keyFiles.find(
          (file: FileObject) => file.filename == data.keyedPath
        )
        ...
        if (existing == null) {
          ...
          const vectorFile = vectorFiles.find(
            (file: VectorStoreFile) => file.id == existing.id
          )
    ...
    keyFiles.forEach((file: FileObject) => {
      ...
      if (
        data == null &&
        removeFiles.find((f: FileData) => f.keyedPath == file.filename) == null
      ) {
    ...
    let removeVectorFilesPromises = removeFileIds
      .map((id: string | undefined) =>
        vectorFiles.find((file: VectorStoreFile) => file.id == id)
      )
      .filter((file: VectorStoreFile | undefined) => file != null)
      .map((file: VectorStoreFile | undefined) =>
    ...
    if (removeVectorFilesPromises.length > 0) {
      core.info(
        `Removing ${removeVectorFilesPromises.length} files from vector store`
      )
    ...
    if (removeFiles.length > 0) {
      ...
      let removeFilesPromises = removeFiles.map((file: FileData) =>
        file.id ? openai.files.del(file.id) : Promise.resolve(null)
      )
    ...
    if (addToVectorStoreFiles.length > 0) {
      ...
      for (let i = 0; i < addToVectorStoreFiles.length; i += 500) {
        ...
        await openai.beta.vectorStores.fileBatches.create(vectorStore.id, {
          file_ids: batch.map((file: FileData) => file.id!)
        })
      }
